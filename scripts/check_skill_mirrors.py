...
This repo keeps agent instructions twice: `.claude/skills/` for Claude and
`.agents/skills/` for Codex, plus `CLAUDE.md` and `AGENTS.md`. Both adapter
trees are gitignored workstation-local state, so nothing else compares them,
and a stale copy does not merely lag — it teaches a procedure the live copy
warns against. On 2026-07-29 KGLite's `.agents/skills/release` was 194 lines
behind and still taught the version-bump belief that had broken its 0.15.0
release, sitting in the file a Codex session would follow (doctrine `R7`).

Here the split is one-directional and mechanical: `CLAUDE.md` and
`.claude/skills/` are the **authority**, `AGENTS.md` and `.agents/skills/` are
**generated** by `scripts/gen_agents_adapter.py`. So a drift report here is
never "hand-port the difference across" — it is either regenerate (staleness)
or edit the authority and regenerate (an improvement someone put in the wrong
file). The only difference the two sides may carry is the name of the
conventions file each one points at.
"""
...
def normalise(text: str) -> str:
    ...
    Skill *paths* are deliberately not substituted: a step that names both
    `.claude/skills/` and `.agents/skills/` means both, in either tree, and
    rewriting one into the other manufactures a diff that is not there.
    """
...
def check_root_pair() -> list[str]:
    ...
    `AGENTS.md` and `.agents/` are gitignored local adapters, so a fresh clone
    has neither and there is genuinely nothing to compare. They install as one
    unit though: once *either* half is present the root pair must be present
    and in sync, otherwise deleting one file would turn the check green — the
    precise silence it exists to prevent.
    """
    ...
    return [
        ...
        "    -> CLAUDE.md is the authority; run `make sync-agents` (never hand-edit AGENTS.md)",
    ]
...
def check_skill_trees() -> list[str]:
    ...
    for rel in sorted(claude & agents):
        ...
        if a != c:
            al, cl = a.splitlines(), c.splitlines()
            # Count the length difference too: zip() stops at the shorter side,
            # so a pure append would otherwise be reported as "~0 lines".
            drift = sum(1 for x, y in zip(al, cl) if x != y) + abs(len(al) - len(cl))
            problems.append(
                f"{rel}: trees differ beyond the CLAUDE.md/AGENTS.md rename "
                f"(~{drift} lines) — .claude is the authority; run `make sync-agents`"
            )
